...
SET @IdTruckDate = DATEADD(DAY,-1,DATEADD (MONTH  , -6 , @IdCurrent )) --testing purpose -6 isnted of CERO
...
SET @idFinDate =  dateadd(week, datediff(week, 0, @idFinDate), 0) --truncar a fin smeana
 ...
 SELECT distinct @as_WHID as WHSE_ID,
  ...
  FROM [ONL_MX_TRAFIGURA].[dbo].[TFG_WMS_AUDIT] a, 
	...
	[ONL_MX_TRAFIGURA].[dbo].[TFG_DL_OUTBOUND] o
  ...
  and c.ClientId in (2, 4 )---TRAFMEX  and TRAF PTE
